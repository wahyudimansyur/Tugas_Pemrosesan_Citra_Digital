...
#if defined (HAVE_OCTAVE_DIM_VECTOR_XELEM)
#  define OCT_DIM_VECTOR_XELEM xelem
#else
#  define OCT_DIM_VECTOR_XELEM elem
#endif

namespace octave_image_pkg
{
    class connectivity
    {
      public:
...
template<class T, class P>
T
octave_image_pkg::connectivity::create_padded (const T& image, const P& val) const
{
...
template<class T>
void
octave_image_pkg::connectivity::unpad (T& image) const
{
  const octave_idx_type pad_ndims = std::min (mask.ndims (), image.ndims ());
...
template<class P>
P
octave_image_pkg::connectivity::min_value (void)
{
...
template<class T, class P>
void
octave_image_pkg::connectivity::set_padding (const dim_vector& size,
                                          const dim_vector& padded_size,
                                          T& im, const P& val)
{
  ...
  const octave_idx_type* lengths_v = lengths.data ();

  const octave_idx_type row_stride = size.OCT_DIM_VECTOR_XELEM (0);
  ...
  fill = [&] (const octave_idx_type dim) -> void
  {
    ...
    if (dim == 0)
      ...
      for (octave_idx_type i = 0; i < size.OCT_DIM_VECTOR_XELEM (dim); i++)
        fill (dim -1);
